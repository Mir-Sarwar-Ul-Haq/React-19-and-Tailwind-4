...
const IMac = () => {
  ...
  const getCurrentModel = () =>
    imacModels.find(
      (model) => model.label.toLowerCase() === selectedColor.toLowerCase()
    );
  return (
      ...
      <div className="absolute md:top-1/2 top-3/4 md:left-4 left-12 transform -translate-y-1/2 flex flex-col lg:space-y-2 space-y-1 bg-gray-50">
        {imacModels.map((imac, index) => (
          <div key={index} className="flex items-center space-x-2">
            ...
            />
            {visibleLabel === imac.label && (
              <span className="text-sm">{imac.label}</span>
            )}
          </div>
        ))}
        <button
